...
class SphereTest {
    ...
    public void testFindIntersections() {
        ...
        final var result2 = sphere.findIntersections(new Ray(p0500, v001));
        assertNotNull(result2, "Can't be empty list");
        assertEquals(1, result2.size(), "Wrong number of points");
        assertEquals(List.of(p001), result2, "Ray starts inside the sphere");
        ...
        final var result4 = sphere.findIntersections(new Ray(new Point(1,0,1),new Vector(1,1,0)));
        ...
        final var result5 = sphere.findIntersections(new Ray(new Point(3,0,0), new Vector(2,0,0)));
        assertNotNull(result4, "Can't be empty list");
        ...
        final var result6 = sphere.findIntersections(new Ray(new Point(1,1,0), new Vector(1,0,0)));
        ...
        final var result7 = sphere.findIntersections(new Ray(new Point(1,0,0), new Vector(2,0,0)));
        ...
        assertNull(sphere.findIntersections(new Ray(new Point(2,0,0), new Vector(3,0,0))), "begin of ray does not count");
        // TC26: Ray starts after sphere (0 points)
        assertNull(sphere.findIntersections(new Ray(new Point(3,0,0), new Vector(4,0,0))), "begin of ray does not count");
        // **** Group 3: Ray's line is tangent to the sphere (all tests 0 points)
        // TC31: Ray starts before the tangent point
        assertNull(sphere.findIntersections(new Ray(new Point(2,-1,0), new Vector(2,0,0))), "begin of ray does not count");
        // TC32: Ray starts at the tangent point
        assertNull(sphere.findIntersections(new Ray(new Point(2,0,0), new Vector(2,1,0))), "begin of ray does not count");
        // TC33: Ray starts after the tangent point
        assertNull(sphere.findIntersections(new Ray(new Point(2,1,0), new Vector(2,2,0))), "begin of ray does not count");
        // **** Group 4: Special cases


        // TC42: Ray's starts inside, ray is orthogonal to ray start to sphere's center line
        final var result8 = sphere.findIntersections(new Ray(new Point(1.6,0,0), new Vector(1.6,0.8,0)));
